                ...
                if re.match("^.+@(\[?)[a-zA-Z0-9-.]+.([a-zA-Z]{2,3}|[0-9]{1,3})(]?)$", email.get()):# validating the email
                    if password.get() == repassword.get(): # checking both password match or not
                     # if u enter in this block everything is fine just enter the values in database
                        ...
                        if gender.get() == 2:
                            gender_value = 'female'
                        connection = pymysql.connect(host="localhost", user="root",passwd="", database="edumate3") # database connection
                        ...
                        Button(screen1, text='Proceed to Login ->', width=20, font=("Open Sans", 9,'bold'), bg='brown', fg='white',command=screen1.destroy).place(x=170, y=565) # buttonto navigate back to login page
                    else:
...
def student_new_record(): 
    ...
    Label(screen4, text="", bg='white',width='86', height='1').grid(row=1,columnspan=4)
...
def enter_new_record(entryField, semester): 
    ...
    if found == 0: 
        if semester.get() == '--0--': 
            messagebox.showerror("Error", "Please select your current semester", parent=screen4) # displaying message for invalid details  r
        else: 
            # enter new record 
            connection = pymysql.connect(host="localhost", user="root", passwd="", database="edumate3") # database connection 
            cursor = connection.cursor()     
...
def student_records(): 
    ...
    Label(screen3, text="", bg='white',width='86', height='1').grid(row=1,columnspan=4)
...
def fetch_record(semester): 
    Label(screen3, text="", bg='#174873',width='100', height='30').place(x=20, y=190)
    #Label(text="", bg='#174873').pack()
    if semester == '--0--': 
...
def welcome_page(student_info):
     ...
     #Message(screen2, text='" Some people dream of accomplishing great things. Others stay awake and make it happen. "\n\n - By Some Night Owl', width='180', font=("Helvetica", 10, 'bold', 'italic'), fg='white', bg='#174873', anchor = 'w').place(x=10, y=100)
     photo = PhotoImage(file="C:\\Users\\Hp\\Desktop\\welcome_page.png") # opening left side image - Note: Ifimage is in same folder then no need to mention the full path
     label = Label(screen2, image=photo, text="") # attaching image to the label
     label.place(x=0, y=78)
     label.image = photo # it is necessary in Tkinter to keep a instance of image to display image in label
     Message(screen2, text='" Some people dream of accomplishing great things. Others stay awake and make it happen. "\n\n - By Some Night Owl', width='180', font=("Helvetica", 10, 'bold', 'italic'), fg='white', bg='#174873', anchor = 'w').place(x=10, y=100)
     #photo1 = PhotoImage(file="C:\\Users\\pic.png") # opening right side image - Note: If image is in same folder then no need to mention the full path
...
def register():
     ...
     Label(screen1, text="University:", font=("Open Sans", 11, 'bold'), fg='white',bg='#174873').place(x=150, y=310)
...
def main_screen():
     ...
     Label(text="", bg='#174873').pack() # for leaving a space in between
     Label(screen, text="", bg='#174873',width='70', height='30').place(x=45, y=120) # blue background in middle of window
